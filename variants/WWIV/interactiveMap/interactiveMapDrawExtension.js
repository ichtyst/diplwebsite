...
function extension(drawFunction, order, fromTerrID, toTerrID, terrID){ //as the draw functions are only local functions of interactiveMap.draw(), the current draw function have to be sent
    switch (order) {
...
function WrapArrowX(fromTerrID, toTerrID, terrID){
    ...
    var start = {x: parseInt(Territories.get(fromTerrID).smallMapX), y: parseInt(Territories.get(fromTerrID).smallMapY)};
    var end = {x: parseInt(Territories.get(toTerrID).smallMapX), y: parseInt(Territories.get(toTerrID).smallMapY)};
    ...
    if(typeof terrID !== 'undefined'){
        // If I have two arrows check which one to point to:
	if (!((Territories.get('warpFrom1').smallMapX == Territories.get('warpFrom2').smallMapX) && (Territories.get('warpFrom1').smallMapY == Territories.get('warpFrom2').smallMapY)))
	{	
            var unit = {x: Territories.get(terrID).smallMapX, y: Territories.get(terrID).smallMapY};
            ...
            if(dist1 < dist2) {
                Territories.set('warpFrom2', Territories.get('warpFrom1'));
                Territories.set('warpTo2', Territories.get('warpTo1'));
            } else {
                Territories.set('warpFrom1', Territories.get('warpFrom2'));
                Territories.set('warpTo1', Territories.get('warpTo2'));
            }
	...
	else
        {
            ...
            Territories.set('warpTerr1', Territories.get('warpFrom1'));
            Territories.set('warpFrom1', Territories.get('warpTo1'));
 
            Territories.set('warpTerr2', Territories.get('warpFrom2'));
            Territories.set('warpFrom2', Territories.get('warpTo2'));
